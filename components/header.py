...
def render_header():
    ...
    # Barra de busca
    # Os widgets já salvam seu valor automaticamente no session_state pela key.
    # Não reatribuímos manualmente — isso causava o StreamlitAPIException.
    col1, col2, col3 = st.columns([4, 1, 1])
    with col1:
        st.text_input("", placeholder="🔍  Buscar produto, marca ou categoria...",
                      label_visibility="collapsed", key="search_query")
    with col2:
        st.selectbox("", ["Todos os marketplaces", "Mercado Livre",
                          "Amazon", "Shopee", "KaBuM!", "Pichau"],
                     label_visibility="collapsed", key="marketplace_filter")
    with col3:
        st.selectbox("", ["Maior desconto", "Menor preço",
